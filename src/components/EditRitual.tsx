...
const EditRitual: React.FC<EditRitualProps> = ({ 
  ...
  // Get habit IDs that are already used as triggers (excluding current ritual)
  const usedTriggerHabitIds = existingRituals
    .filter(r => r.id !== ritual?.id && r.trigger.type === 'habit')
    .map(r => (r.trigger as HabitTrigger).habitId);
  ...
  useEffect(() => {
    if (ritual) {
      ...
      if (ritual.trigger.type === 'time') {
        ...
        const habit = existingHabits.find(h => h.id === ritual.trigger.habitId);
        setSelectedHabit(habit || null);
      }
      setFrequency(ritual.frequency);
      setReward(ritual.reward || '');
    }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto modal-scroll">
        <div className="flex items-center justify-between p-6 border-b">
          <h2 className="text-xl font-bold text-gray-900">Edit Ritual</h2>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            ...
            {triggerType === 'time' ? (
              ...
              <HabitPicker
                habits={existingHabits.filter(h => h.id !== ritual.id)}
